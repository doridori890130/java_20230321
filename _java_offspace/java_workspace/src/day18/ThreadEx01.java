...
public class ThreadEx01 {
	...
	public static void main(String[] args) {
		/*	Thread(쓰레드)  
		 * 	실행중인 프로그램 (process)이 OS로부터
		 * 	메모리를 할당받아 process 상태가 됨
		 * 
		 ...
		 * 	CPU는 시간을 잘게 쪼개서 thread들을 수행하면 ㅏ용자들은 동시에 처리되는 듯한 효과
		 * 	thread는 각각 자신만의 작업 공간을 가짐(context)
		 * 	각 thread 사이에 공유하는 자원이 있을 수 있음.(자바에서는 static instance)
		 * 
		 * 	여러 thread가 공유하는 자원중 경쟁이 발생하는 부분을 critical section(임계영역)
		 * 	critical section 에 대한 동기화(순차적 수행)를 구현하여 오류를 막음.
		 * 
		 * 	Thread 구현방식 상속, 구현하는 방식 2가지	
		 * 
		 * */
