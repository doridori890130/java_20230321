...
public class MapEx01 {
	...
	public static void main(String[] args) {
		...
		// 1. 저장공간을 어디에다가 둘지
			// - 1. 맴버변수
			// - 2. main(외부에서 매개변수로 받는 형태) => 매개변수로 받는 형태
			// - 3. 메서드내부에 생성 -> 리턴의 형태
		// 2. 매개변수와 리턴타입, 메서드명을 결정 => interface 생성
		//1번형태
		//ex.make();
//		Iterator<String> it = ex.map.keySet().iterator();
//		Iterator<String> it = ex.getmap.keySet().iterator(); -> 타클래스의 경우 get으로 접근
//		while(it.hasNext()) {
...
//		}
	...
	//1. 맴버변수로 map이 있는 형태 (공용) 리턴x, 받을것도 없음.
	public void make() {
		while(map.size()<size) {
	...
	//3. 내부에서 map 생성하는 형태(리턴이 map인 형태)
