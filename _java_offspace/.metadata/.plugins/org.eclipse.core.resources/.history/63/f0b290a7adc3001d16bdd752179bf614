...
public class ProductManager {
	...
	public void updateProductPrice() {
		System.out.println("수정할 품목 : ");
		String updateP = scan.next();
		System.out.println("수정할 가격 : ");
		int updatePrice = scan.nextInt();
		
		for(int i= 0; i < ProductList.size() ; i++) {
			if(ProductList.get(i).getProductName().equals(updateP)){
				ProductList.get(i).setProductPrice(updatePrice);
	...
	public void deleteProduct() {
		System.out.println("삭제할 품목 :");
		String deleteProduct = scan.next();
		
		for(int i=0; i< ProductList.size();i++) {
			if(ProductList.get(i).getProductName().equals(deleteProduct)) {
